...
class JeopardyCompetitionController extends Controller
{
    ...
    public function register(){
        ...
        return view('chemecompe-regis')->with(compact('institution', $institution));
    }
    ...
    public function createStep3(Request $request, $count){
        ...
        $member2 = $request->session()->get('member2');
        $member3 = $request->session()->get('member3');
        return view('couponcheme', ['count', $count])
        ->with(compact('papercompe', $papercompe))
        ->with(compact('team_leader', $team_leader))
        ->with(compact('member1', $member1))
        ->with(compact('member2', $member2))
    ...
    public function postCreateStep1(Request $request){
        ...
        if(!strcmp($request->place3, "Other")){
            $this->validate($request, [
                'other_place3' => 'required'
            ]);
            $validatedData['place3'] = $request->other_place2;
        }
        ...
        if (empty($request->session()->get('member3'))){
            if ($request->member3 && $request->place3){
                $member3 = new Member();
                $member3->fill([
                    'nama' => $validatedData['member3'],
                    'institution' => $validatedData['place3'],
                    'jenis_member' => "Member",
                    'biaya_pendaftaran' => $biaya
                ]);

                $request->session()->put('member3', $member3);
        ...
        $total_biaya = $biaya_leader + $biaya_member1 + $biaya_member2 + $biaya_member3;
        ...
        return redirect()->route('cheme-competition-regis-voucher-get', ['count'=>$count]);
    }
    ...
    public function postCreateStep2(Request $request){
        ...
        $request->session()->forget('member3');
    ...
    public function postCreateStep3(Request $request){
        ...
        $this->validate($request, [
            ...
            'member3' => 'nullable|exists:App\Models\Voucher,kode_voucher'
        ],
        [
            ...
            'member3.exists' => 'The voucher code for third member is invalid'
        ]
        );
        ...
        if($member3) {
            $member3->biaya_pendaftaran=$biaya;
            $biaya_member3 = $member2->biaya_pendaftaran;
            $count++;
        }
        else $biaya_member3 = 0;
        ...
        if ($request->member3){
            $voucher = Voucher::whereKode_voucher($request->member3)->first();
            if (!strcmp($voucher->jenis_voucher, "Organisasi")){
                if (strcmp($voucher->institution->nama_institusi, $member3->institution)){
                    return redirect()->back()->withErrors(['member3_roadshow' => 'This voucher is not eligible for third member.']);
                }
            }
            if (!strcmp($voucher->jenis_voucher, "Roadshow")){
                if (Carbon::now()->toDateString()>$voucher->expired_date){
                    return redirect()->back()->withErrors(['member3_roadshow' => 'This voucher is already expired.']);
                }
            }
            $voucher_member3 = $voucher->id;
            if($papercompe->origin) $voucher->besar_potongan = $voucher->besar_potongan/10000;
            $member3->biaya_pendaftaran = $member3->biaya_pendaftaran-$voucher->besar_potongan;
            $biaya_member3 = $member3->biaya_pendaftaran;
        }

        $papercompe->total_pembayaran = $biaya_leader + $biaya_member1 + $biaya_member2 + $biaya_member3;
        ...
        if($member3){
            $member3->fill([
                'voucher_id' => $voucher_member3
            ]);
        }

        // dd($team_leader, $member1, $member2, $papercompe);
        return redirect()->route('cheme-competition-total', ['count' =>$count]);
    }
}
