    ...
    <div class="pt-32 md:grid md:grid-cols-5 pb-96 md:pb-32 md:w-full">
        <div class="md:col-span-2 md:col-start-1 pl-1 md:pl-24 flex flex-col items-start md:w-full">
            <h1 class="hidden md:block md:absolute py-5 md:py-0.5 md:relative text-2xl md:text-5xl font-bold">
                Payment of {{ $payment->comperegis->nama_tim }}
            ...
            <div class="w-72 md:w-full pt-2 md:pt-0 md:pr-5 z-10">
                <label for="" class="pb-2 pl-4 text-base md:text-xl font-medium">Payment Method</label>
                <div class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form">
                    {{ $payment->method }}
                </div>
                <label for="" class="pb-2 pl-4 text-base md:text-xl font-medium">Name on card</label>
                <div class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form">
                    {{ $payment->name_card }}
                </div>
                <label for="" class="pb-2 pl-4 text-base md:text-xl font-medium">Payment date</label>
                <div class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form">
                    {{ $payment->payment_date }}
                </div>
                
                <label for="" class="pb-2 pl-4 text-base md:text-xl font-medium">Payment Proof</label>
                <img src="/comperegis_payment/{{ $payment->filename }}" alt="Payment">
                
                {{-- <div class="flex flex-col pt-20">
                    <a href="{{ route('competition-data', $payment->comperegis->jenis) }}" class="pt-2 md:pl-5">
                        <button onclick='javascript:return confirm("Are you sure want to validate {{ $payment->comperegis->nama_tim }} payment?");' href="" type="" class="px-40 md:px-40 bg-gradient-to-l from-green-300 to-yellow-400 py-3 rounded-full text-white text-lg font-semibold shadow-lg">
                            Validate
                        </button>
                    </a>
                </div> --}}
