    ...
    <div class="pt-32 md:grid md:grid-cols-5 pb-96 md:pb-32 md:w-full">
        <div class="md:col-span-2 md:col-start-1 pl-1 md:pl-24 flex flex-col items-start md:w-full">
            ...
            <div class="w-72 md:w-full pt-2 md:pt-0 md:pr-5 z-10">
                <form action="{{route('voucher-update', $voucher->id)}}" method="POST" class="flex flex-col pt-10 md:pt-12">
                    @csrf
                    <label for="kode_voucher" class="pb-2 pl-4 text-base md:text-xl font-medium">Kode Voucher</label>
                    <input value="{{ $voucher->kode_voucher }}" name="kode_voucher" type="text" class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form" placeholder="">
                    @error('kode_voucher')
                    ...
                    <input value="{{ $voucher->besar_potongan }}" name="besar_potongan" type="text" class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form" placeholder="">
                    ...
                    <select name="jenis_voucher" for="jenis_voucher" class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form">
                        ...
                        <option @if ($voucher->jenis_voucher == "Campus Ambassador")
                            selected
                        @endif class="py-1 font-sans">Campus Ambassador</option>
                        <option @if ($voucher->jenis_voucher == "Roadshow")
                            selected
                        @endif class="py-1 font-sans">Roadshow</option>
                        <option @if ($voucher->jenis_voucher == "Organisasi")
                            selected
                        @endif class="py-1 font-sans">Organisasi</option>
                    </select>

                    <label for="expired_date" class="pt-4 pb-2 pl-4 text-xl font-medium">Expired Date (Must be filled for Roadshow Voucher)</label>
                    <input value="{{ $voucher->expired_date }}" name="expired_date" type="date" class="input border-b-2 focus:border-form border-form py-1 px-4 pt-4 w-full text-sm bg-transparent">
                    @error('expired_date')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
                        </div>
                    ...
                    <select name="institution" for="institution" class="outline-none rounded-full border border-form py-1 px-4 w-96 md:w-80 text-sm focus:ring-2 focus:ring-form">
                        ...
                        @foreach ($institution as $t)
                            <option 
                            @if ($voucher->institution)
                                @if ($t->nama_institusi == $voucher->institution->nama_institusi)
                                    selected
                                @endif 
                            @endif
                            class="py-1 font-sans">{{ $t->nama_institusi }}</option>
                        @endforeach
                    </select>
                    @error('institution')
                        <div class="text-red-500 mt-2 text-sm">
